...
def extract_single_zip(zip_path, out_dir):
    ...
    with zipfile.ZipFile(zip_path, "r") as zf:
        ...
        with ThreadPoolExecutor(max_workers=MAX_THREADS) as ex:
            ...
            for _ in tqdm(as_completed(futures), total=len(futures), desc=f"{zname}", unit="file", ncols=80):
                pass
    return len(members)

def main():
    # Put HF cache on fast local disk too
    os.environ["HF_ENDPOINT"] = "https://hf-mirror.com"
    os.environ["HF_HOME"] = "/tmp/.cache/hf"
    os.environ["HF_HUB_ENABLE_HF_TRANSFER"] = "1"

    # === LOCAL NVMe paths (ephemeral) ===
    SNAPSHOT_ROOT = "/tmp/hf_snapshot"
    OUT_DIR = "/tmp/new_cache_dataset/train"   # final images live here

    os.makedirs(SNAPSHOT_ROOT, exist_ok=True)
    os.makedirs(OUT_DIR, exist_ok=True)

    print(">>> NOTE: /tmp is ephemeral; checkpoints should be saved in /workspace.")
    print(">>> Downloading HF snapshot to local NVMe ...")
    ...
    with ThreadPoolExecutor(max_workers=MAX_ZIP_WORKERS) as executor:
        list(executor.map(lambda z: extract_single_zip(z, OUT_DIR), zip_files))

    print("\n>>> Counting extracted images ...")
    total_jpg = sum(1 for f in os.listdir(OUT_DIR) if f.lower().endswith((".jpg", ".jpeg", ".png")))
    print(f"\n>>> Extraction complete!")
